...
def group_pixels(colored_pixels):
	...
	for index in range(len(colored_pixels)):
		if colored_pixels[index,0] <= x+num_pixels and colored_pixels[index,0] >= x-num_pixels \
			and colored_pixels[index,1] <= y+num_pixels and colored_pixels[index,1] <= y+num_pixels:
...
def calc_circle(c,nr1,nr2,car_pos):
	carx = car_pos[0]
	cary = car_pos[1]
	x1 = c[nr1][0]
	y1 = c[nr1][1]
	...
	Hx = x1+np.cos(alpha)*500						#calculation of the "Lot"
	Hy = y1+np.sin(alpha)*500
	l_m = -1/((Hy-y1)/(Hx-x1))
	l_b = y1-l_m*x1 
...
class visual_gps:	
	...
	def callback(self,data):
		...
		if len(c)>=3:	#at least 3 lamps are visible
			car_pos = [img_gray.shape[1]/2, img_gray.shape[0]/2]
			#circle 1		
			[Mx1,My1,r1] = calc_circle(c,0,1,car_pos) 
			cv2.circle(img_res,(int(Mx1),int(My1)),int(r1),[200,0,0],5) #img,center,radius,color,thickness
			#circle 2		
			[Mx2,My2,r2] = calc_circle(c,0,2,car_pos) 
			cv2.circle(img_res,(int(Mx2),int(My2)),int(r2),[0,200,0],5) #img,center,radius,color,thickness
			...
			#select right intersection
			right_intersection = select_intersection(car_pos,intersections)
			cv2.circle(img_res,(int(right_intersection[0]),int(right_intersection[1])),2,[0, 0, 150],5) #img,center,radius,color,thickness
			car_pos = right_intersection
		
			#calculate the world coordinates and publish them
			[car_x,car_y,yaw] = calc_world_coor(c,car_pos)
